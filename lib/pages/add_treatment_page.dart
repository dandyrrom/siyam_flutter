...
class _AddTreatmentPageState extends State<AddTreatmentPage> {
    ...
    try {
      ...
      if (widget.prefillItemId != null) {
        ...
        for (final item in items) {
          if (item.itemId == widget.prefillItemId) {
            match = item;
            break;
          }
        }
        ...
        if (match != null && match.currentUsableStockQty > 0) {
          ...
          final safeQty = requestedQty > 0 ? requestedQty : 1.0;
  ...
  bool _petHasMedicalRecord(
    ...
    return _existingTreatments.any(
      (treatment) => treatment.petId == pet.petId,
    );
  }

  bool get _selectedPetHasMedicalRecord => _petHasMedicalRecord(
        _selectedPet,
      );
  ...
  bool get _hasEmptyDraft {
    return _itemDrafts.any(
      (draft) => draft.item == null,
    );
  }

  bool get _hasUnusedInStockItems {
    final used = _usedItemIds;
  ...
  Future<void> _save() async {
    ...
    if (_selectedPet!.status == PetStatus.adopted ||
        _selectedPet!.status == PetStatus.deceased) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'This animal is no longer eligible to receive treatment.',
    ...
    final treatmentItems = _itemDrafts
        .map(
          (draft) => _inputFromItem(
            draft.item!,
            qty: draft.qty,
          ),
        )
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    TextButton.icon(
                      onPressed: !_saving && _canAddItem ? _addItemRow : null,
                      icon: const Icon(
                        Icons.add,
                        size: 16,
                      ),
                ...
                LayoutBuilder(
                  builder: (
                    ...
                    final administeredField = TextFormField(
                    ...
                    final dateField = InkWell(
                      ...
                      onTap: _saving ? null : _pickDateAdministered,
...
class _TreatmentItemDraftCard extends StatefulWidget {
  ...
  @override
  State<_TreatmentItemDraftCard> createState() =>
      _TreatmentItemDraftCardState();
}

class _TreatmentItemDraftCardState extends State<_TreatmentItemDraftCard> {
  ...
  void initState() {
    ...
    final selected = widget.draft.item;

    if (selected != null) {
      _searchCtrl.text = _itemDisplayText(
        selected,
      );
    }
  ...
  String _itemDisplayText(
    ...
    final availability = '${formatQty(item.currentUsableStockQty)} '
  ...
  void _handleSearchTextChanged(
    ...
    final expected = _itemDisplayText(
      selected,
    );
  ...
  Widget _buildItemSearch(
    List<InventoryItem> selectableItems,
  ) {
      ...
      builder: (
        ...
        return RawAutocomplete<InventoryItem>(
          ...
          optionsBuilder: (
            ...
            return selectableItems.where(
              (item) {
                final searchable = '${item.itemName} '
                        '${item.itemCategory} '
  ...
  Widget _buildDoseField(
    ...
    final unit = selectedItem == null
        ? null
        : (selectedItem.dispenseUnitAbbr ??
            selectedItem.packageUnitAbbr ??
            selectedItem.purchaseUnitAbbr);
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        labelText: unit == null ? 'Dose' : 'Dose ($unit)',
      ),
      onChanged: (
        value,
      ) {
      ...
      validator: (
        ...
        if (selectedItem.stockOutIsDeductible && qty > _maxDoseQty) {
          return 'Only '
  ...
  Widget build(
    ...
    final selectedItem = widget.draft.item;
    ...
    final selectableItems = widget.items
        .where(
          (item) =>
              item.currentUsableStockQty > 0 &&
              (selectedItem?.itemId == item.itemId ||
                  !widget.usedItemIds.contains(
                    item.itemId,
                  )),
        )
        .toList()
    ...
    final isLowStock = selectedItem?.stockLevel == StockLevel.low;
